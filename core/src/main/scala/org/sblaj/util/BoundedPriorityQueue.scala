 ...
 * scala.collection.mutable.PriorityQueue. By default PriorityQueue instances
 * in Scala are unbounded. This trait modifies the original PriorityQueue's
 * enqueue methods such that we only retain the top K elements. The top K
 * elements are defined by an implicit Ordering[A].
 */
...
object BoundedPriorityQueue {
   ...
   */
  def apply[A](maximum: Int, keepMinPriority: Boolean = false)
    (implicit ord: Ordering[A]): BoundedPriorityQueue[A] = {
